    ...
    # needed: ncol(B) = ncol(DD)
{
    ...
    #!
    if(hat1){
        Q <- qr.qy(model$qr, diag(1, nrow = nrow(model$qr$qr), ncol = model$qr$rank))
        Q <- Q[1:length(y), , drop = FALSE]
        Q1 <- (1 / sqrt(w1)) * Q
        Q2 <- sqrt(w1) * Q
        hat <- tcrossprod(Q1, Q2)
        diag.hat.ma1 <- diag(hat)
    ...
    list(a=a1, diag.hat.ma=diag.hat.ma1, weight = w1, fitted = z1,B=B,P=P, hat = hat)  
}
